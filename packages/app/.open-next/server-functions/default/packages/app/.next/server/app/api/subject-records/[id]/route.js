(() => {
    ...
    (e.modules = {
      ...
      70897: (e, t, r) => {
        ...
        async function c(e, { params: t }) {
          try {
            let { id: r } = await t,
              s = await d.j.api.getSession({ headers: await (0, u.b3)() });
            if (!s)
              return Response.json({ error: "Unauthorized" }, { status: 401 });
            let { name: a } = await e.json();
            if (!a)
              return Response.json(
                { error: "Subject name is required" },
                { status: 400 }
              );
            let i = await o.im.Subject.getRecordById(r);
            if (!i || i.userId !== s.user.id)
              return Response.json(
                {
                  error:
                    "Subject not found or you don't have permission to update it",
                },
                { status: 404 }
              );
            if (
              a !== i.name &&
              (await o.im.Subject.getRecordByName(a, s.user.id))
            )
              return Response.json(
                { error: "Subject with this name already exists" },
                { status: 400 }
              );
            let n = await o.im.Subject.update(r, { name: a });
            if (!n)
              return Response.json(
                { error: "Failed to update subject" },
                { status: 500 }
              );
            return Response.json({ subject: n });
          } catch (e) {
            return (
        ...
        async function l(e, { params: t }) {
          try {
            ...
            if (!r)
              return Response.json({ error: "Unauthorized" }, { status: 401 });
            return (
              await o.im.Subject.removeByUser(e, r.user.id),
        ...
        let p = new a.AppRouteRouteModule({
            definition: {
              ...
              page: "/api/subject-records/[id]/route",
              pathname: "/api/subject-records/[id]",
              filename: "route",
              bundlePath: "app/api/subject-records/[id]/route",
            },
            resolvedPagePath:
              "/home/aiz/dev/redentor/brainbytes/packages/app/src/app/api/subject-records/[id]/route.ts",
  ...
  t.C(e);
  var r = (e) => t((t.s = e)),
    s = t.X(0, [50, 151, 680, 208], () => r(70897));
  module.exports = s;
})();
